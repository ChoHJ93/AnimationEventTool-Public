...
namespace GizmoTool
{
    public class PrimitiveMeshFactory
    {
        ...
        public static Mesh BoxWireframe(Vector3 size)
        {
            Mesh mesh = new Mesh();
        ...
        public static Mesh BoxFlatShaded(Vector3 size)
        {
            float width = size.x;
            float height = size.y;
            float depth = size.z;
            ...
            Vector3 v0 = new Vector3(-width * .5f, -height * .5f, depth * .5f);
            Vector3 v1 = new Vector3(width * .5f, -height * .5f, depth * .5f);
            Vector3 v2 = new Vector3(width * .5f, -height * .5f, -depth * .5f);
            Vector3 v3 = new Vector3(-width * .5f, -height * .5f, -depth * .5f);
            Vector3 v4 = new Vector3(-width * .5f, height * .5f, depth * .5f);
            Vector3 v5 = new Vector3(width * .5f, height * .5f, depth * .5f);
            Vector3 v6 = new Vector3(width * .5f, height * .5f, -depth * .5f);
            Vector3 v7 = new Vector3(-width * .5f, height * .5f, -depth * .5f);
        ...
        static public Mesh CreateTorus(float radius, float thickness, int radiusSegmentCount, int sideCount)
        {
            Mesh mesh = new Mesh();


            Vector3[] vertices = new Vector3[(radiusSegmentCount + 1) * (sideCount + 1)];
            float _2pi = Mathf.PI * 2f;
            for (int seg = 0; seg <= radiusSegmentCount; seg++)
            {
                int currSeg = seg == radiusSegmentCount ? 0 : seg;

                float t1 = (float)currSeg / radiusSegmentCount * _2pi;
                Vector3 r1 = new Vector3(Mathf.Cos(t1) * radius, 0f, Mathf.Sin(t1) * radius);

                for (int side = 0; side <= sideCount; side++)
                {
                    int currSide = side == sideCount ? 0 : side;

                    Vector3 normale = Vector3.Cross(r1, Vector3.up);
                    float t2 = (float)currSide / sideCount * _2pi;
            ...
            for (int seg = 0; seg <= radiusSegmentCount; seg++)
            {
                for (int side = 0; side <= sideCount - 1; side++)
                {
                    int current = side + seg * (sideCount + 1);
                    int next = side + (seg < (radiusSegmentCount) ? (seg + 1) * (sideCount + 1) : 0);
        ...
        static public Mesh CreateArc(Vector3 center, Vector3 startPoint, Vector3 axis, float radius, float angle, int segmentCount)
        {
            Mesh mesh = new Mesh();

            Vector3[] vertices = new Vector3[segmentCount + 2];

            Vector3 startVector = (startPoint - center).normalized * radius;
            for (int i = 0; i <= segmentCount; i++)
            {
                float rad = (float)i / segmentCount * angle;
                Vector3 v = Quaternion.AngleAxis(rad * 180f / Mathf.PI, axis) * startVector;
                vertices[i] = v + center;
            }
